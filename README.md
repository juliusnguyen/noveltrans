...
| 笔趣阁 (bqg5.com) | `https://m.bqg5.com/4_4217/` (nhận cả `www.bqg5.com/<id>/` và link chương — dán kiểu nào cũng ra cùng một truyện). **Site này chặn trình duyệt máy tính**: mở trên máy tính thì ra lỗi 404, chỉ điện thoại mới xem được — app tự gửi định danh trình duyệt điện thoại nên cứ dán link vào là chạy, **không cần trình duyệt, không cần đăng nhập hay cookie**. Bản điện thoại (`m.`) chia mục lục thành nhiều trang và cắt mỗi chương làm mấy trang nhỏ, nên app tự đổi sang bản `www`: lấy trọn mục lục trong **1 lần tải** và mỗi chương cũng chỉ **1 lần**, nội dung y hệt. Nội dung là **tiếng Trung giản thể** (khác ba site trên là phồn thể), tải xong dịch ở tab 2 như bình thường. Đầu trang mục lục của site có khối “chương mới nhất” trùng lặp; app chỉ lấy khối 正文 và **báo lỗi chứ không lưu danh sách sai** nếu nhận ra thứ tự bị đảo. |
| 小說543 (novel543.com) | `https://www.novel543.com/0802691255` (nhận cả link mục lục `<id>/dir` và link chương — dán kiểu nào cũng ra cùng một truyện). **Có kiểm tra Cloudflare** — app tự mở trình duyệt để vượt qua, cần Google Chrome hoặc `playwright install chromium`; **giữ cửa sổ đó mở tới khi tải xong** và **không tải song song nhiều luồng**. Không cần đăng nhập hay cookie. Nội dung là **tiếng Trung phồn thể**, tải xong dịch ở tab 2 như bình thường. **Site cắt phần lớn chương làm 2 trang nhỏ**, app tự ghép lại nên mỗi chương vẫn đầy đủ — nhưng vì vậy tải cả truyện lâu gần gấp đôi các site khác, cứ để chạy. Site này chặn gắt hơn các site khác nên **app tự giãn nhịp tải tối thiểu 3 giây/trang** (nếu bạn đã đặt chậm hơn thì giữ nguyên mức của bạn); đây là lý do nó chậm, không phải treo. Trang mục lục có kèm khối “chương mới nhất” trùng lặp ở đầu; app chỉ lấy khối mục lục thật và **báo lỗi chứ không lưu danh sách sai** nếu danh sách không liền mạch từ chương 1 — gặp lỗi đó thì quét lại sau. |
| 天天看小說 (ttks.tw) | `https://ttks.tw/novel/chapters/daqiandingliutaohunfumaye/index.html` (nhận cả link chương `<slug>/<số>.html` — dán kiểu nào cũng ra cùng một truyện). **Có kiểm tra Cloudflare** — app tự mở trình duyệt để vượt qua, cần Google Chrome hoặc `playwright install chromium`; **giữ cửa sổ đó mở tới khi tải xong** và **không tải song song nhiều luồng**. Không cần đăng nhập hay cookie. Nội dung là **tiếng Trung phồn thể**, tải xong dịch ở tab 2 như bình thường. Trang mục lục của site vừa là trang giới thiệu vừa là mục lục đầy đủ nên quét cả truyện chỉ tốn **một lần mở trang**. Đầu mục lục có kèm khối “chương mới nhất” trùng lặp và **xếp ngược**; app lọc theo số thứ tự chương trong link nên không bị lẫn, và **báo lỗi chứ không lưu danh sách sai** nếu danh sách không liền mạch từ chương 1 — gặp lỗi đó thì quét lại sau. Mỗi chương site chèn một dòng quảng cáo tên miền (viết bằng ký tự lạ, mỗi chương một kiểu) vào giữa bài; app tự bỏ dòng đó ngay lúc tải nên **bản gốc lưu lại đã sạch**, không cần sửa tay. |
...
- **Codex CLI**: dịch bằng [Codex CLI](https://github.com/openai/codex) của OpenAI, dùng gói ChatGPT đã đăng nhập — **không cần API key**, nhưng tính vào hạn mức sử dụng Codex của gói. Cài bằng `npm i -g @openai/codex`, rồi chạy `codex login` một lần trong Terminal. Lệnh mặc định (sửa trong Cài đặt): `codex exec --skip-git-repo-check --sandbox read-only --ephemeral` — `exec` là chế độ không tương tác, `--skip-git-repo-check` vì app chạy Codex ngoài thư mục git, `--sandbox read-only` vì dịch không cần ghi file, `--ephemeral` để không lưu mỗi chương thành một phiên trong `~/.codex/sessions`. Thiếu `exec`, `--skip-git-repo-check` hay `--sandbox` thì app tự thêm. Chọn model ở ô **Model** trên tab Dịch (để trống = model mặc định trong `~/.codex/config.toml`); muốn nhanh hơn thì thêm `-c model_reasoning_effort="low"` vào lệnh. Nội dung chương gửi qua stdin nên chương dài không bị cắt, kể cả trên Windows. Codex cũng dùng được cho **Viết lại văn phong**, **Kiểm tra chất lượng** và phần AI của tab Video.
...
- Chỉ dành cho **bản dịch tiếng Việt**; engine phải là **CLI Agent, Codex CLI, Claude API
  hoặc LM Studio** (Google chỉ dịch được, không viết lại được nên không xuất hiện trong danh
